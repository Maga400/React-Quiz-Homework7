...
export default function Quiz() {
    ...
    function nextQuestion()
    {
        ...
        else
        {
            if(check.current != null)
            {
                e.current.target.checked = false;
                check.current = null;
            }
    ...
    function finishQuiz()
    {
        ...
        alert(`Siz ${questions.length} sualdan ${count.current} duz cavab vermisiniz.Duzgun cavab verdiyiniz suallar: ${trueAns.current}`);
        window.location.reload();
    }

  return (
    <section>
        <section style={{backgroundColor:"white",color:"red",margin:"auto",borderRadius:"20px",width:"40%",fontSize:"1.5em",marginTop:"2%"}}>
        ...
        <section style={{backgroundColor:"white",borderRadius:"20px",width:"80%",margin:"auto",marginTop:"3%",padding:"0px 20px 20px 20px",display:"flex",justifyContent:"start"}}>
            <section style={{width:"60%"}}>
                ...
                <section style={{display:"flex",padding:"10px 0px 10px 40px",marginTop:"30px",borderRadius:"10px",border:"3px solid lightgrey",width:"100%",fontSize:"1.5em"}}>
                    <input id='1' type='radio' name='answer' onClick={(e) => checkColor(true,question.answer1,e,"1",question.answer1)}></input>
                    <h1 style={{marginLeft:"10px"}}>{question.answer1}</h1> 
                </section>
                <section style={{display:"flex",padding:"10px 0px 10px 40px",marginTop:"30px",borderRadius:"10px",border:"3px solid lightgrey",width:"100%",fontSize:"1.5em"}}>
                    <input id='2' type='radio' name='answer' onClick={(e) => checkColor(true,question.answer1,e,"2",question.answer2)}></input>
                    <h1 style={{marginLeft:"10px"}}>{question.answer2}</h1>
                </section>
                <section style={{display:"flex",padding:"10px 0px 10px 40px",marginTop:"30px",borderRadius:"10px",border:"3px solid lightgrey",width:"100%",fontSize:"1.5em"}}>
                    <input id='3' type='radio' name='answer' onClick={(e) => checkColor(true,question.answer1,e,"3",question.answer3)}></input>
                    <h1 style={{marginLeft:"10px"}}>{question.answer3}</h1>
                </section>
                <section style={{display:"flex",padding:"10px 0px 10px 40px",marginTop:"30px",borderRadius:"10px",border:"2px solid lightgrey",width:"100%",fontSize:"1.5em"}}>
                    <input id='4' type='radio' name='answer' onClick={(e) => {checkColor(false,question.answer1,e,"4",question.answer4)}}></input>
                    <h1 style={{marginLeft:"10px"}}>Bunlardan hec biri</h1>
                </section>
            ...
            <section style={{borderRadius:"10px",border:"2px solid lightgrey",width:"35%",marginLeft:"80px",marginTop:"60px",padding:"20px"}}>
                ...
                <section style={{marginTop:"50px"}}>
                    {
                        questions.map((q) => (<Circle data={q}></Circle>))
                    }
                </section>
